...
class App:
	def __init__(self,video_source = "videoTest.mp4"):   #testVideo.mp4     #videoTest.mp4

		self.appname="Video Surveillance"
		self.window=Tk()
		self.window.title(self.appname)
		self.window.geometry('1350x800')
		self.window.state("zoomed")
		self.window["bg"]='#141414'

		image1=Image.open("bgImages/bg6.jpg")
		image1 = image1.resize((1250,700), Image.ANTIALIAS)
		photo1=ImageTk.PhotoImage(image1)
		photo_label1=Label(image=photo1).place(x=245,y=65)
		photo_label1


		self.video_source=video_source
		self.vid=myvideocapture(self.video_source)
		self.label=Label(self.window,text=self.appname,font=("bold",20),bg='black',fg='white').pack(side=TOP,fill=BOTH)
		self.canvas=Canvas(self.window,height=700,width=700,bg='#141414')
		...
		self.tree.heading("#1", text="Missing Person-ID")
		self.tree.column("#1", minwidth=0, width=200, stretch=NO)

		self.tree.heading("#2", text="Name")
		self.tree.column("#2", minwidth=0, width=160, stretch=NO)
		...
		ttk.Style().configure("Treeview.Heading",font=('Helvetica', 13,'bold'), foreground="black", relief="flat")

		self.tree.place(x=730,y=90)
	...
	def viewdetail(self,a):
		...
		label_bloodgroup = Label(self.window, text="Address",bg="#382273",fg='yellow',width=20,font=("bold", 12))
		label_bloodgroup.place(x=930,y=550)
		label_body = Label(self.window, text="Contact Number",bg="#382273",fg='yellow',width=20,font=("bold", 12))
		label_body.place(x=930,y=580)
		label_nat = Label(self.window, text="BodyMark",bg="#382273",fg='yellow',width=20,font=("bold", 12))
	...
	def getProfile(self,id):
	    conn=sqlite3.connect("missing.db")
	    cmd="SELECT ROWID, Name,Address,ContactNumber FROM People where ROWID="+str(id)
...
class myvideocapture:
	...
	def __init__(self,video_source="videoTest.mp4"):
